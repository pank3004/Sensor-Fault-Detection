...
class ModelTrainer: 
    ...
    def evalute_models(self, X_train, y_train, X_test, y_test, models): 
        try: 
            report={}
    ...
    def get_best_model(self,X_train:np.array, 
                      ...
                      y_test:np.array): 
        try: 
            model_raport:dict=self.evalute_models(X_train=X_train,
                                                  y_train=y_train,
                                                  X_test=X_test,
                                                  y_test=y_test,
                                                  models=self.models)
            ...
            best_model_score=max(sorted(model_raport.values()))

            ## to get best model

            best_model_name=list(model_raport.keys())[
                list(model_raport.values()).index(best_model_score)
            ]  
    ...
    def initiate_model_trainer(self, train_array, test_array):
        try:
            ...
            logging.info(f"Splitting training and testing input and target feature completed")

            logging.info(f"Extracting model config file path")

            model_report: dict = self.evalute_models(X_train=X_train, y_train=y_train, X_test=X_test, y_test=y_test, models=self.models)
